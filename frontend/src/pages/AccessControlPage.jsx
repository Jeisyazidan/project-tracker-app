          ...
          <div key={role} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:20, marginBottom:16, boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {ALL_PERMISSIONS.map(p => {
                ...
                return (
                  <label key={p.key} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', background: checked ? '#f0fdf4' : 'var(--surface2)', border:`1px solid ${checked ? '#bbf7d0' : 'var(--border)'}`, borderRadius:8, cursor:'pointer', transition:'background .15s' }}>
